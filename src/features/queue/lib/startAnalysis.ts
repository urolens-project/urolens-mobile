...
/**
 * @description "Begin Analysis" is when a specimen becomes In Progress. The server owns
 * specimen status (sync rule 1: server wins), so the change must reach it — otherwise
 * the next pull sets the specimen back to ASSIGNED. Online, tell the server now; if it
 * can't be reached — or there's no connection — queue it for the next sync and flip the
 * local status so the Queue badge updates immediately.
 *
 ...
 * @param params - The specimen to start, its server id, and current connectivity.
 */
