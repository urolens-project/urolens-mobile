...
/**
 * The one status a Queue sample counts as. Every active sample is exactly one
 * of these, and the card badge, the stats counts, the status filters and the
 * sort order all use this same rule so they can never disagree:
...
/**
 * Returned, then In Progress, then Assigned; newest first within each group.
 * Returns a new array.
 */
